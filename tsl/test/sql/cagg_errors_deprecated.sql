...
CREATE MATERIALIZED VIEW mat_m1 WITH (timescaledb.continuous, timescaledb.finalized = true, timescaledb.myfill = 1)
as
...
--valid PG option
CREATE MATERIALIZED VIEW mat_m1 WITH (timescaledb.continuous, timescaledb.finalized = true, check_option = LOCAL )
as
select * from conditions , mat_t1 WITH NO DATA;

--non-hypertable
CREATE MATERIALIZED VIEW mat_m1 WITH (timescaledb.continuous, timescaledb.finalized = true)
as
...
--aggregate with DISTINCT (supported with new cagg format)
CREATE MATERIALIZED VIEW mat_m1 WITH (timescaledb.continuous, timescaledb.finalized = true)
AS
...
--aggregate with FILTER (supported with new cagg format)
CREATE MATERIALIZED VIEW mat_m1 WITH (timescaledb.continuous, timescaledb.finalized = true)
AS
Select time_bucket('1week', timec),
...
-- aggregate with filter in having clause (supported with new cagg format)
CREATE MATERIALIZED VIEW mat_m1 WITH (timescaledb.continuous, timescaledb.finalized = true)
AS
...
-- ordered set aggr (supported with new cagg format)
CREATE MATERIALIZED VIEW mat_m1 WITH (timescaledb.continuous, timescaledb.finalized = true)
AS
...
-- supported with new format of caggs
CREATE MATERIALIZED VIEW mat_m1 WITH (timescaledb.continuous, timescaledb.finalized = true)
AS
...
DROP MATERIALIZED VIEW mat_m1;
-- userdefined aggregate without combine function (supported with new cagg format)
...
--using ORDER BY in view defintion (supported with new cagg format)
CREATE MATERIALIZED VIEW mat_m1 WITH (timescaledb.continuous, timescaledb.finalized = true)
AS
Select sum(humidity), avg(temperature::int4)
from conditions
...
CREATE VIEW continuous_aggs_errors_tbl1 WITH (timescaledb.continuous, timescaledb.finalized = true) AS
...
WITH (timescaledb.continuous, timescaledb.finalized = true)
as
...
CREATE MATERIALIZED VIEW measurements_summary WITH (timescaledb.continuous, timescaledb.finalized = true) AS
...
SELECT add_job(
       ...
       '1 hour'::interval,
       check_config => '_timescaledb_internal.policy_refresh_continuous_aggregate_check'::regproc,
       config => '{"end_offset": null, "start_offset": "1 fortnight", "mat_hypertable_id": 19}');
-- ... this one because it has a bad value for end_offset
SELECT add_job(
       '_timescaledb_internal.policy_refresh_continuous_aggregate'::regproc,
       '1 hour'::interval,
       check_config => '_timescaledb_internal.policy_refresh_continuous_aggregate_check'::regproc,
       config => '{"end_offset": "chicken", "start_offset": null, "mat_hypertable_id": 19}');
\set ON_ERROR_STOP 1
...
SELECT alter_job(:job_id,
       config => '{"mat_hypertable_id": 19, "end_offset": "chicken", "start_offset": "1 fortnights"}');
SELECT alter_job(:job_id,
       config => '{"mat_hypertable_id": 19, "end_offset": "chicken", "start_offset": "1 week"}');
\set ON_ERROR_STOP 1
...
create materialized view i2980_cagg with (timescaledb.continuous, timescaledb.finalized = true) AS SELECT time_bucket('1h',time), avg(7) FROM i2980 GROUP BY 1;
...
CREATE MATERIALIZED VIEW  i2980_cagg2 with (timescaledb.continuous, timescaledb.compress, timescaledb.finalized = true)
AS SELECT time_bucket('1h',time), avg(7) FROM i2980 GROUP BY 1;

--this one succeeds
CREATE MATERIALIZED VIEW  i2980_cagg2 with (timescaledb.continuous, timescaledb.finalized = true)
...
CREATE MATERIALIZED VIEW cagg1 WITH(timescaledb.continuous, timescaledb.finalized = true) AS SELECT time_bucket('1h',_ts_meta_min_1) FROM :INTERNALTABLE GROUP BY 1;
